...
class Scanner(object):
    ...
    def process_url_iocs(self, backend_cfg: dict) -> None:
        ...
        2. Re-ingests them as new files into the Strelka pipeline via emit_file
        3. Maintains parent-child relationships automatically
        ...
        try:
            ...
            for ioc in url_iocs:
                ...
                try:
                    # Emit the URL as a new text file.
                    # Providing the 'url_file' flavor allows easy assignment 
                    # to the HTTPX scanner in backend.yaml
                    self.emit_file(
                        data=url_value.encode('utf-8'),
                        name=f"___urls___{url_value}",
                        flavors=['url_file']
                    )
